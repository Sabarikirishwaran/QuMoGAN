...
class Solver(object):
    ...
    def test(self):
        ...
        trained_weights = (-0.038017816, 0.688643007, -0.038848318, -0.122427502, -0.435173561, -0.936836542, 0.997042291, 0.771221968, 0.561501131, 0.708183667, 0.139691279, -0.8726815, -0.916428431, -0.451635658)
        gen_weights = torch.tensor(trained_weights)
        sample_list = [torch.cat([torch.tensor(gen_circuit_1(gen_weights)), torch.tensor(gen_circuit_2(gen_weights))])
        ...
        with torch.no_grad():
            ...
            logits_fake, features_fake = self.D(edges_hat, None, nodes_hat)
            g_loss_fake = - torch.mean(logits_fake)
            print("Prob.of.real.mol: {:.2f}%".format( (1+g_loss_fake)*100 ))

            # Fake Reward
            (edges_hard, nodes_hard) = self.postprocess((edges_logits, nodes_logits), 'hard_gumbel')
            edges_hard, nodes_hard = torch.max(edges_hard, -1)[1], torch.max(nodes_hard, -1)[1]
            mols = [self.data.matrices2mol(n_.data.cpu().numpy(), e_.data.cpu().numpy(), strict=False)
                    for e_, n_ in zip(edges_hard, nodes_hard)]

        img = mols2grid_image(mols, 4)
        img.save('molecules/gen.drug.png')

        # mols.save(('molecules/gen.drug.mol'))
        # m = Chem.MolFromMolFile('molecules/gen.drug.mol')
